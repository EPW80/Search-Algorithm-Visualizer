...
import { DFSAlgorithm, DFS } from '../DFS';
import {
  createSimpleGrid,
  createGridWithWalls,
  createNoPathGrid,
...
describe('DFSAlgorithm', () => {
  describe('Basic path finding', () => {
    ...
    it('should visit nodes in DFS order (depth-first)', () => {
    ...
    it('should find a path (not necessarily shortest)', () => {
      ...
      expect(hasPath(result.pathArray)).toBe(true);
      // DFS may not find the shortest path, just a valid path
  ...
  describe('Edge cases and validation', () => {
    ...
    it('should handle grid with start at bottom-right', () => {
      const grid = createEmptyGrid(5, 5);
  ...
  describe('Legacy function wrapper', () => {
    it('should work with legacy DFS function', () => {
      const { grid, start, end } = createSimpleGrid();
    ...
    it('should produce same results as class-based approach', () => {
      ...
      const functionResult = DFS(grid, start, end);
  ...
  describe('Weighted cells (DFS ignores weights)', () => {
  ...
  describe('Stack-based behavior', () => {
    it('should use stack behavior (LIFO)', () => {
      // Create a simple grid where DFS behavior can be observed
      const grid = createEmptyGrid(3, 3);
      const start: [number, number] = [1, 1]; // Center
      const end: [number, number] = [0, 0]; // Top-left
      setStart(grid, start[0], start[1]);
      setEnd(grid, end[0], end[1]);
      ...
      expect(hasPath(result.pathArray)).toBe(true);
      // DFS will explore one direction fully before backtracking
      expect(result.visited.length).toBeGreaterThan(2);
    });
  });
});
