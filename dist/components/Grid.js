...
import { useState } from 'react'; // Add useState import here
import Cell from './Cell';
import { useGrid } from '../context/GridContext';
import '../styles/Grid.css';
var Grid = function () {
    var _a = useGrid(), grid = _a.grid, updateCellState = _a.updateCellState; // Using GridContext to manage state
    ...
    var handleMouseEnter = function (row, col) {
        ...
        if (draggedNodeType === 'start') {
            var _a = findStartOrEndNode('start'), startRow = _a[0], startCol = _a[1]; // Destructure the tuple
            updateCellState(row, col, { isStart: true });
            updateCellState(startRow, startCol, { isStart: false });
        }
        else if (draggedNodeType === 'end') {
            var _b = findStartOrEndNode('end'), endRow = _b[0], endCol = _b[1]; // Destructure the tuple
